                ...
                <div class="card shadow-sm border-0 rounded-3">
                    ...
                    <div class="card-body p-5">
                        <form method="POST" action="{{ route('store_user.telegram') }}" enctype="multipart/form-data">
                            ...
                            <input type="hidden" name="password" value="{{ $telegram_user->phone }}">
                            <input type="hidden" name="email" value="{{ $telegram_user->email }}">

                            <!-- Display the Telegram user information -->
                            <div class="mb-3">
                                <label for="display_name" class="form-label">{{ __('Telegram User Information') }}</label>
                                <div class="form-control-plaintext">
                                    ...
                                    <p><strong>{{ __('Avatar URL:') }}</strong> <a href="{{ $telegram_user->avatar }}" target="_blank">{{ $telegram_user->avatar }}</a></p>
                                </div>
                            </div>
